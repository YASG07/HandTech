...
def mostrarAnalisisSintactico2(tokens, scrollAnalisis):
    scrollAnalisis.config(state="normal")
    scrollAnalisis.delete(1.0, END)
    for asintactico in tokens:
        idMethod,tipo,idParam,bloque = asintactico
        # scrollAnalisis.insert(INSERT, f"{token_type} {token_value}\n")
        scrollAnalisis.insert(INSERT, f" {idMethod} {tipo} {idParam} {{\n{bloque}\n}}")
        pass  # Esto es solo para mostrar un marcador de posición
    scrollAnalisis.config(state="disabled")

def analisisSintacticoxd():
    cadena = scroll_text_widget.get_text()
    if len(cadena) > 0:
        lexer.input(cadena)
        as_tok = []
        for tok in parser:
            as_tok.append((tok.type, tok.value, tok.lineno, tok.lexpos))
        mostrarAnalisisSintactico2(as_tok, scrollAnalisis)  # Llamada a la función para mostrar el análisis sintáctico
    else:
        mb.showwarning("ERROR", "Debes escribir código")
...
analisis.add_command(label="Sintactico", command=analisisSintactico3)  
